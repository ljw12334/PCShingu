...
int clock = 0;
String heatsink = request.getParameter("heatsink");
...
try {
	clock = Integer.parseInt(request.getParameter("clock"));
	
	price = Integer.parseInt(request.getParameter("price"));
	stock = Integer.parseInt(request.getParameter("stock"));

	if (name == null || name.isBlank() || manufacturer == null || manufacturer.isBlank() || kind == null || kind.isBlank() ||
		heatsink == null || heatsink.isBlank() || clock <= 0) {
		%>
		<script>
	...
	try (
			...
			ResultSet rs = stmt.executeQuery("select * from list_ram where name='" + name + "'");
			) {
...
try (
	...
	if (rs.next()) {
		...
		stmt.executeUpdate(String.format(
				"update list_ram set manufacturer='%s', kind='%s', clock=%d, heatsink='%s'",
				manufacturer, kind, clock, heatsink) +
					// 공통 속성
					String.format(
		...
		stmt.executeUpdate(String.format(
				"insert into list_ram (name, manufacturer, kind, clock, heatsink" +
				// 공통 속성
				", price, image, stock) " +
				// 값
				"values('%s', '%s', '%s', %d, '%s'" + 
				// 공통 속성
				", %d, '%s', %d)",
				name, manufacturer, kind, clock, heatsink,
				// 공통 속성
				price, image, stock));
	}
	
	response.sendRedirect("../management_ram.jsp");
	return;
