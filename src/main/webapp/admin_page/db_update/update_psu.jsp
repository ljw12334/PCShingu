...
try {
	...
	if (name == null || name.isBlank() || manufacturer == null || manufacturer.isBlank() || standard == null || standard.isBlank() ||
		cable == null || cable.isBlank() || power <= 0) {
		%>
		<script>
	...
	try (
			...
			ResultSet rs = stmt.executeQuery("select * from list_psu where name='" + name + "'");
			) {
...
try (
	...
	if (rs.next()) {
		...
		stmt.executeUpdate(String.format(
				"update list_psu set manufacturer='%s', standard='%s', cable='%s' power=%d",
				manufacturer, standard, cable, power) +
					// 공통 속성
					String.format(
		...
		stmt.executeUpdate(String.format(
				"insert into list_psu (name, manufacturer, standard, cable, power" +
				// 공통 속성
				", price, image, stock) " +
				// 값
				"values('%s', '%s', '%s', '%s', %d" + 
				// 공통 속성
				", %d, '%s', %d)",
				name, manufacturer, standard, cable, power,
				// 공통 속성
				price, image, stock));
	}
	
	response.sendRedirect("../management_psu.jsp");
	return;
