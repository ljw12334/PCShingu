...
int core = 0;
int thread = 0;
String builtInGraphics = request.getParameter("built-in-graphics");
...
try {
	clockDef = Float.parseFloat(request.getParameter("clock-def"));
	clockMax = Float.parseFloat(request.getParameter("clock-max"));
	core = Integer.parseInt(request.getParameter("core"));
	thread = Integer.parseInt(request.getParameter("thread"));
	price = Integer.parseInt(request.getParameter("price"));
	stock = Integer.parseInt(request.getParameter("stock"));

	if (name == null || name.isBlank() || manufacturer == null || manufacturer.isBlank() || gen == null || gen.isBlank() ||
		modifier == null || modifier.isBlank() || socket == null || socket.isBlank() ||
		core <= 0 || thread <= 0 || clockDef <= 0 || clockMax <= 0) {
		%>
		<script>
	...
	try (
			...
			ResultSet rs = stmt.executeQuery("select * from list_cpu where name='" + name + "'");
			) {
...
try (
	...
	if (rs.next()) {
		...
		stmt.executeUpdate(String.format(
				"update list_cpu set manufacturer='%s', gen='%s', modifier='%s', socket='%s', clock_def='%f', clock_max='%f', core=%d, thread=%d, built_in_graphics='%s'",
				manufacturer, gen, modifier, socket, clockDef, clockMax, core, thread, builtInGraphics) +
					// 공통 속성
					String.format(
		...
		stmt.executeUpdate(String.format(
				"insert into list_cpu (name, manufacturer, gen, modifier, socket, clock_def, clock_max, core, thread, built_in_graphics" +
				// 공통 속성
				", price, image, stock) " +
				// 값
				"values('%s', '%s', '%s', '%s', '%s', %f, %f, %d, %d, '%s'" + 
				// 공통 속성
				", %d, '%s', %d)",
				name, manufacturer, gen, modifier, socket, clockDef, clockMax, core, thread, builtInGraphics,
				// 공통 속성
				price, image, stock));
	}
	
	response.sendRedirect("../management_cpu.jsp");
	return;
