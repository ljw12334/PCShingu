...
int size = 0;
String dram = request.getParameter("dram");
...
try {
	...
	try (
			...
			ResultSet rs = stmt.executeQuery("select * from list_ssd where name='" + name + "'");
			) {
...
try (
	...
	if (rs.next()) {
		...
		stmt.executeUpdate(String.format(
				"update list_ssd set manufacturer='%s', kind='%s', size=%d, dram='%s'",
				manufacturer, kind, size, dram) +
					// 공통 속성
					String.format(
		...
		stmt.executeUpdate(String.format(
				"insert into list_ssd (name, manufacturer, kind, size, dram" +
				// 공통 속성
				", price, image, stock) " +
				// 값
				"values('%s', '%s', '%s', %d, '%s'" + 
				// 공통 속성
				", %d, '%s', %d)",
				name, manufacturer, kind, size, dram,
				// 공통 속성
				price, image, stock));
	}
	
	response.sendRedirect("../management_ssd.jsp");
	return;
