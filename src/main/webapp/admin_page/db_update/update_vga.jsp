...
int fanCount = 0;
int length = 0;
String backplate = request.getParameter("backplate");
int power = 0;
...
try {
	hdmi = Integer.parseInt(request.getParameter("hdmi"));
	dp = Integer.parseInt(request.getParameter("dp"));
	rgb = Integer.parseInt(request.getParameter("rgb"));
	dvi = Integer.parseInt(request.getParameter("dvi"));
	vram = Integer.parseInt(request.getParameter("vram"));
	fanCount = Integer.parseInt(request.getParameter("fan-count"));
	length = Integer.parseInt(request.getParameter("length"));
	power = Integer.parseInt(request.getParameter("power"));
	price = Integer.parseInt(request.getParameter("price"));
	stock = Integer.parseInt(request.getParameter("stock"));

	if (name == null || name.isBlank() || kind == null || kind.isBlank() || manufacturer == null || manufacturer.isBlank() ||
		chipset == null || chipset.isBlank() || backplate == null || backplate.isBlank() ||
		vram <= 0 || length <= 0) {
		%>
		<script>
	...
	try (
			...
			ResultSet rs = stmt.executeQuery("select * from list_vga where name='" + name + "'");
			) {
...
try (
	...
	if (rs.next()) {
		...
		stmt.executeUpdate(String.format(
				"update list_vga set kind='%s', manufacturer='%s', chipset='%s', vram=%d, hdmi=%d, dp=%d, rgb=%d, dvi=%d, fan_count=%d, length=%d, backplate='%s', power=%d",
				kind, manufacturer, chipset, vram, hdmi, dp, rgb, dvi, fanCount, length, backplate, power) +
					// 공통 속성
					String.format(
		...
		stmt.executeUpdate(String.format(
				"insert into list_vga (name, kind, manufacturer, chipset, vram, hdmi, dp, rgb, dvi, fan_count, length, backplate, power" +
				// 공통 속성
				", price, image, stock) " +
				// 값
				"values('%s', '%s', '%s', '%s', %d, %d, %d, %d, %d, %d, %d, '%s', %d" + 
				// 공통 속성
				", %d, '%s', %d)",
				name, kind, manufacturer, chipset, vram, hdmi, dp, rgb, dvi, fanCount, length, backplate, power,
				// 공통 속성
				price, image, stock));
	}
	
	response.sendRedirect("../management_vga.jsp");
	return;
